...
export function DataClient() {
  ...
  async function importJson(ev: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (
      !confirm(
        'Import akan MENGGANTIKAN seluruh produk, penjualan, belanja, pengeluaran & jurnal. Lanjutkan?'
      )
    )
      return;
    setBusy('import');
  ...
  async function resetAll() {
    if (
      !confirm(
        'HAPUS SEMUA DATA operasi (produk, penjualan, belanja, pengeluaran, jurnal, konsinyasi, member)? Akun pengguna tetap ada. Tindakan ini TIDAK BISA DIBATALKAN!'
      )
    )
      return;
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <div className="card flex flex-col justify-between p-4">
            <div>
              ...
              <p className="mb-3 text-xs text-slate-500 dark:text-slate-400">
                Unduh salinan lengkap seluruh database (produk, kas, transaksi, member) untuk arsip / pindah server.
              </p>
            </div>
          ...
          <div className="card flex flex-col justify-between p-4">
            <div>
              ...
              <p className="mb-3 text-xs text-slate-500 dark:text-slate-400">
                Pulihkan data dari file backup JSON Kopontren. Data operasi saat ini akan ditimpa dengan data backup.
              </p>
            </div>
            <input
              ref={fileRef}
          ...
          <div className="card flex flex-col justify-between border-rose-200 bg-rose-50/20 p-4 dark:border-rose-950 dark:bg-rose-950/10">
            <div>
              ...
              <p className="mb-3 text-xs text-slate-500 dark:text-slate-400">
                Kosongkan seluruh data operasional (penjualan, stok, kas, konsinyasi). Akun login pengurus tetap aman.
              </p>
            </div>
            <button
