            ...
            {cart.map((l, idx) => {
              ...
              >
                ...
                <div className="mt-2 flex items-center justify-between gap-2">
                  <div className="flex shrink-0 items-center overflow-hidden rounded-lg border border-slate-300 bg-white dark:border-navy-600 dark:bg-navy-800">
                    <button
                      type="button"
                      className="grid h-11 w-11 place-items-center text-slate-600 transition active:scale-90 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-navy-700 sm:h-9 sm:w-9"
                      onClick={() => setQty(l.product.id, l.qty - 1)}
                      title={l.qty <= 1 ? 'Hapus item' : 'Kurangi jumlah'}
                      aria-label="Kurangi jumlah"
                    >
                      <Minus className="h-5 w-5" />
                    </button>
                    <span
                      className="min-w-10 text-center text-sm font-bold tabular-nums text-slate-800 dark:text-slate-100"
                ...
                {admin && (
                  <div className="mt-2 flex items-center justify-end gap-2 text-xs text-slate-500 dark:text-slate-400">
                    ...
                    <input
                      className="input h-11 w-24 px-2 text-right text-sm sm:h-9"
                      type="number"
                      inputMode="numeric"
                      value={l.price}
